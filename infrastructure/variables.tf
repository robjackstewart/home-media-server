...
variable "arc_cluster_name" {
  type        = string
  description = "The name this cluster is registered under in Azure Arc (Microsoft.Kubernetes/connectedClusters)."
  default     = "home-media-server"
}

variable "git_revision" {
  type        = string
  description = "The commit SHA Flux should deploy, via the Arc Flux configuration's git_repository.reference_value. No default deliberately - every apply must pass one explicitly (`task recreate` passes `git rev-parse HEAD`; CI passes `$GITHUB_SHA`), so infrastructure changes in this same apply (Key Vault secrets, the Arc/Flux setup) always land before Flux is told to deploy a chart that might depend on them."
}
